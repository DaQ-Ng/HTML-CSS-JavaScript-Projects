...
function updateStrengthMeter(password) {
  ...
  const hasUppercase = /[A-Z]/.test(password); //Check if has uppercase
  ...
  //Enfore minimum score for short password
  if (passwordLength < 8) {
...
function createRandomPassword(
  ...
  if (includeUppercase) {
    allCharacter += uppercaseLetters;
  }
  if (includeLowercase) {
    allCharacter += lowercaseLetters;
  }
  if (includeNumbers) {
    allCharacter += numberCharacters;
  }
  if (includeSymbols) {
    allCharacter += symbolCharacters;
  }
